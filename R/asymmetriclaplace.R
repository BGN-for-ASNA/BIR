#' @title AsymmetricLaplace distribution wrapper.
#' @param loc 0.0
#' @param scale 1.0
#' @param asymmetry 1.0
...
#' @export
bi.dist.asymmetriclaplace=function(loc=0.0, scale=1.0, asymmetry=1.0, validate_args=py_none(), name='x', obs=py_none(), mask=py_none(), sample=FALSE, seed=0, shape=c(), event=0, create_obj=FALSE) { 
     shape=do.call(tuple, as.list(as.integer(shape)))
     seed=as.integer(seed);
     .bi$dist$asymmetriclaplace(loc=loc,  scale= scale,  asymmetry= asymmetry,  validate_args= validate_args,  name= name,  obs= obs,  mask= mask,  sample= sample,  seed= seed,  shape= shape,  event= event,  create_obj= create_obj)
}
