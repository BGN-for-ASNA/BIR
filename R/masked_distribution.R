#' @title MaskedDistribution distribution wrapper.
...
#' @export
bi.dist.maskeddistribution=function(base_dist, mask, name='x', obs=py_none(), sample=FALSE, seed=0, shape=c(), event=0, create_obj=FALSE) { 
     shape=do.call(tuple, as.list(as.integer(shape)))
     seed=as.integer(seed);
     .bi$dist$maskeddistribution(base_dist,  mask,  name= name,  obs= obs,  sample= sample,  seed= seed,  shape= shape,  event= event,  create_obj= create_obj)
}
