#' @title ExpandedDistribution distribution wrapper.
...
#' @export
bi.dist.expandeddistribution=function(base_dist, batch_shape=c(), name='x', obs=py_none(), mask=py_none(), sample=FALSE, seed=0, shape=c(), event=0, create_obj=FALSE) { 
     shape=do.call(tuple, as.list(as.integer(shape)))
     seed=as.integer(seed);
     .bi$dist$expandeddistribution(base_dist,  batch_shape= batch_shape,  name= name,  obs= obs,  mask= mask,  sample= sample,  seed= seed,  shape= shape,  event= event,  create_obj= create_obj)
}
