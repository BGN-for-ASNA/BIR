#' @title Delta distribution wrapper.
#' @param v The fixed value that the distribution represents.
#' @param log_density The log probability density of the value v.
#' @param event_dim The number of event dimensions.
...
bi.dist.delta=function(v=0.0, log_density=0.0, event_dim=0, validate_args=py_none(), name='x', obs=py_none(), mask=py_none(), sample=FALSE, seed=0, shape=c(), event=0, create_obj=FALSE) {
     ...
     .bi$dist$delta(v=v,  log_density= log_density,  event_dim= event_dim,  validate_args= validate_args,  name= name,  obs= obs,  mask= mask,  sample= sample,  seed= seed,  shape= shape,  event= event,  create_obj= create_obj)
}
