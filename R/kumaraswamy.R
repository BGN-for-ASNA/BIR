#' @title Kumaraswamy distribution wrapper.
...
bi.dist.kumaraswamy=function(concentration1, concentration0, validate_args=py_none(), name='x', obs=py_none(), mask=py_none(), sample=FALSE, seed=0, shape=c(), event=0, create_obj=FALSE) { 
     ...
     .bi$dist$kumaraswamy(concentration1,  concentration0,  validate_args= validate_args,  name= name,  obs= obs,  mask= mask,  sample= sample,  seed= seed,  shape= shape,  event= event,  create_obj= create_obj)
}
