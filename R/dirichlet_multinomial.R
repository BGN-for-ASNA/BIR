...
bi.dist.dirichlet_multinomial=function(concentration, total_count=1, validate_args=py_none(), name='x', obs=py_none(), mask=py_none(), sample=FALSE, seed=0, shape=c(), event=0, create_obj=FALSE) {
     ...
     .bi$dist$dirichlet_multinomial(jnp$array(concentration),  total_count= jnp$array(as.integer(total_count)),  validate_args= validate_args,  name= name,  obs= obs,  mask= mask,  sample= sample,  seed= seed,  shape= shape,  event= event,  create_obj= create_obj)
}
